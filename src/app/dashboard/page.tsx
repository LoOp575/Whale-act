...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <PageHeader
        title="WhaleCopy AI Dashboard"
        description="Monitor smart wallets, AI signals, and paper trading performance."
      />

      {/* Stat Cards - 5 columns on xl, 3 on lg, 2 on md */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="bg-dark-800/50 backdrop-blur border border-dark-700/40 rounded-xl p-5 hover:border-dark-600/60 transition-all duration-300"
          >
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-medium text-dark-400 uppercase tracking-wide">
                {stat.label}
              </span>
              <div className={`w-8 h-8 rounded-lg ${stat.bgColor} flex items-center justify-center ${stat.accentColor}`}>
                {stat.icon}
              </div>
            </div>
            <p className="text-2xl font-bold text-white mb-1">{stat.value}</p>
            <p className={`text-xs font-medium ${
              stat.changeType === "positive" ? "text-accent-emerald" :
              stat.changeType === "negative" ? "text-accent-rose" :
              "text-dark-400"
            }`}>
              {stat.change}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="/ai-signals" className="text-xs text-whale-400 hover:text-whale-300 transition-colors font-medium">
              View all
            </a>
          </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="/live-activity" className="text-xs text-whale-400 hover:text-whale-300 transition-colors font-medium">
              View all
            </a>
          </div>
            ...
            {activities.slice(0, 7).map((activity) => (
              ...
              >
                ...
                <span className="text-sm font-medium text-dark-200 flex-shrink-0">
                  ${activity.valueUsd >= 1000000
                    ? (activity.valueUsd / 1000000).toFixed(1) + "M"
                    : activity.valueUsd >= 1000
                    ? (activity.valueUsd / 1000).toFixed(0) + "K"
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="/top-wallets" className="text-xs text-whale-400 hover:text-whale-300 transition-colors font-medium">
              View all
            </a>
          </div>
          <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    index === 0 ? "bg-accent-amber/15 border border-accent-amber/25" :
                    index === 1 ? "bg-dark-600/40 border border-dark-500/30" :
                    "bg-dark-700/40 border border-dark-600/20"
                  }`}>
                    <span className={`text-xs font-bold ${
                      index === 0 ? "text-accent-amber" : "text-dark-300"
                    }`}>
                      #{index + 1}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-4 mt-2.5 pt-2.5 border-t border-dark-700/20">
                  ...
                  <span className={`text-[11px] font-medium ml-auto px-1.5 py-0.5 rounded ${
                    ...
                    wallet.riskScore === "Medium" ? "bg-accent-amber/10 text-accent-amber" :
