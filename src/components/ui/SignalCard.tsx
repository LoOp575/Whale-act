...
const SIGNAL_CONFIG: Record<SignalType, { label: string; color: string; bg: string; border: string; dotColor: string }> = {
  BUY: { label: "BUY", color: "text-accent-emerald", bg: "bg-accent-emerald/10", border: "border-accent-emerald/25", dotColor: "bg-accent-emerald" },
  WAIT: { label: "WAIT", color: "text-accent-cyan", bg: "bg-accent-cyan/10", border: "border-accent-cyan/25", dotColor: "bg-accent-cyan" },
  REJECT: { label: "REJECT", color: "text-accent-rose", bg: "bg-accent-rose/10", border: "border-accent-rose/25", dotColor: "bg-accent-rose" },
  EXIT: { label: "EXIT", color: "text-accent-rose", bg: "bg-accent-rose/8", border: "border-accent-rose/20", dotColor: "bg-accent-rose" },
  WARNING: { label: "WARNING", color: "text-red-400", bg: "bg-red-500/8", border: "border-red-500/20", dotColor: "bg-red-400" },
};
...
export function SignalCard({ signal, compact = false }: SignalCardProps) {
  ...
  if (compact) {
    return (
      <div className="p-3.5 rounded-lg bg-dark-850/60 border border-dark-700/30 hover:border-dark-600/50 transition-all">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-dark-700/60 flex items-center justify-center">
              <span className="text-xs font-bold text-white">{signal.token.slice(0, 2)}</span>
            </div>
            <div>
              <span className="text-sm font-semibold text-white">{signal.token}</span>
              <p className="text-[11px] text-dark-500">{signal.timestamp}</p>
            </div>
          </div>
          <span className={`text-[11px] font-semibold px-2 py-1 rounded-md ${config.bg} ${config.color} border ${config.border}`}>
            {config.label}
          </span>
        </div>
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[11px] font-medium ${signal.priceChange24h >= 0 ? "text-accent-emerald" : "text-accent-rose"}`}>
  ...
  return (
    <div className={`relative rounded-xl border p-5 sm:p-6 transition-all duration-200 bg-dark-800/40 hover:bg-dark-800/60 ${config.border}`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`text-xs font-bold ${signal.confidence >= 80 ? "text-accent-emerald" : signal.confidence >= 60 ? "text-accent-amber" : "text-accent-rose"}`}>
            {signal.confidence}%
          </span>
        </div>
        <div className="w-full h-1.5 bg-dark-700/50 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${signal.confidence >= 80 ? "bg-accent-emerald" : signal.confidence >= 60 ? "bg-accent-amber" : "bg-accent-rose"}`}
      ...
      <div className="mt-4 pt-4 border-t border-dark-700/30">
        ...
        <p className={`text-sm font-medium mt-1 ${config.color}`}>{signal.suggestedAction}</p>
      </div>
